...
def create_env(env_id, client_id, remotes, **kwargs):
    if (env_id == 'MiniPacman'):
        return create_minipacman_env(env_id)
    elif (env_id == 'Simple'):
        return create_simple_env(env_id)
...
def create_simple_env(env_id):
    env = Simple('regular', 200)
    env = Vectorize(env)
    env = DiagnosticsInfo(env)
    env = Unvectorize(env)
    return env

def create_minipacman_env(env_id):
